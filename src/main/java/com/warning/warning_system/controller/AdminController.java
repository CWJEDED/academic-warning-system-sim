...
public class AdminController {
    ...
    public ResponseEntity<?> assignCourseToTeacher(@RequestBody Map<String, Object> params) {
        try {
            Integer teaId = (Integer) params.get("teaId");
            String teaName = (String) params.get("teaName");
            String subjectName = (String) params.get("subject");

            if (teaId == null || subjectName == null) {
                return ResponseEntity.badRequest().body("参数不完整");
            }

            // 1. 检查是否已经分配过 (防止重复)
            // 假设 CourseTeacherRepository 中有 findByTeaIdAndSubject 方法，或者使用 Example 查询
            // 这里使用 Example 查询作为通用解法
            CourseTeacher probe = new CourseTeacher();
            probe.setTeaId(teaId);
            probe.setSubject(subjectName);
            boolean exists = courseRepo.exists(org.springframework.data.domain.Example.of(probe));

            if (exists) {
                return ResponseEntity.badRequest().body("该教师已负责【" + subjectName + "】，不可重复分配！");
            }

            // 2. 新增分配记录
            CourseTeacher ct = new CourseTeacher();
            ct.setTeaId(teaId);
            ct.setTeaName(teaName);
            ct.setSubject(subjectName);
            // 默认设置，如需学期可在此扩展
